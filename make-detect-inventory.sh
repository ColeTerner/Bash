...
cd $path
touch detect-inventory && >detect-inventory
...
echo '[all]' > detect-inventory
#echo '5.252.193.80 ansible_user=fface ansible_password=fface-pwd1902 become-user=root ansible_become_pass=fface-pwd1902 ansible_sudo_pass=fface-pwd1902' >> detect-inventory
...
do
	...
	echo "$ip token=' $token '" >> detect-inventory
	i=$((i+1))
done
...
do
	echo $string >> detect-inventory
done

cat detect-inventory
